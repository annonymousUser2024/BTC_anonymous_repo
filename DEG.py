...
class ChargingStation:
    def __init__(self, CS_location, original_index):
        ...
        self.cs_slope = round(random.uniform(-3, 3) , 2)
...
###munkres function to maximize profit
...
###munkres function to minimize cost
def munkres_func(matrix):
    print("-----in munkres function")
    # cost_matrix = make_cost_matrix(matrix, lambda cost: (- cost) if (cost != DISALLOWED) else DISALLOWED)
    m = Munkres()
    ...
    for i in range(start_row, end_row):
        ...
        for j in range(len(matrix[0])):
            ...
            if (robot.state_of_charge - total_e_to_complete_task) < lowest_soc:
                ...
                print("not enough energy so disallowed")
            else:
        ...
        if robot:
            ...
            if closest_CS:
                if is_available:
                    ...
                    # if closest_CS.robot_waiting_to_charge != None:
                    ...
                    robot.allocated_CS = closest_CS.original_index
                    if cs_reaching_time == k:
                        # closest_CS.robots_waiting_to_charge.append(robot_index)
                        ...
                        print("robot", robot)
                        print("closest cs_occupied", closest_CS)
                # need_charge_indexes.remove(rob_index)
...
robot_speed = 1.3 * period_length_sec ##in meter/sec or metter/minute according to period puration. the multiplication with period_length is done to convert it according to the decision period
...
print(f"fixed e {fixed_energy_consumption}")
energy_consumption_while_idle = params.get('energy_consumption_while_idle', None)
print(f"idle e {energy_consumption_while_idle}")
...
while k <= T:
    ...
    # print("--new all tasks list after adding new tasks:--")
    # for task in all_tasks:    
    #     print(task)
    # print(f"#####print all lists after parameter updation at the beginning of k={k}")
...
print("% gained value for DEG:", percentage_gained_valuation) 
print(f"execution time per allocation for DEG: {per_alloc_time}") 

# print(f"battery degradation traces: {bat_deg_traces}") 
...
# print(f"----!!! per period execution time for DEG: {per_period_execution_time}")   

###to create dataframes
df1 = pd.DataFrame()    
df1['DEG_gained_value'] = percentage_gained_valuation
df1['DEG_per_alloc_exec_time'] = per_alloc_time
# df1['DEG_per_peri_exec_time'] = per_period_execution_time

       
###write reults in a csv
bat_deg_data = {'DEG_deg_Rob{}'.format(robot.original_index): bat_deg_perc[robot.original_index] for robot in all_robots}
df2 = pd.DataFrame(bat_deg_data)

soc_data = {'DEG_soc_Rob{}'.format(robot.original_index): state_of_charge_traces[robot.original_index] for robot in all_robots} 
...
File_name1 = f"results_DEG_{experiment_name}.csv"
File_name2 = f"bat_deg_DEG_{experiment_name}.csv"
File_name3 = f"soc_trace_DEG_{experiment_name}.csv"
# File_name4 = f"total_utility_DEG_{experiment_name}.csv"
...
if max_no_of_days == 1:
    ...
    plt.savefig(f"state_of_charge_DEG_{experiment_name}.pdf")
    plt.show()
...
if max_no_of_days > 1:
    ...
    plt.legend()
    # plt.grid(True)
    plt.savefig(f'bat_degradation_DEG_{experiment_name}_{max_no_of_days}_days.pdf')
    plt.show()
